...
def alarm_dashboard():
    try:
        ...
        if start_datetime and end_datetime:
            print(
                f"🔍 날짜 필터링 적용 - 시작: {start_datetime} (타입: {type(start_datetime)})")
            print(
                f"🔍 날짜 필터링 적용 - 종료: {end_datetime} (타입: {type(end_datetime)})")
        ...
        # 날짜 필터가 적용된 경우 샘플 데이터 출력
        if start_datetime and end_datetime and alarms:
            print(f"🔍 조회된 데이터 샘플 (첫 3개):")
            for i, alarm in enumerate(alarms[:3]):
                print(
                    f"  {i+1}. 장비명: {alarm.equip_name}, 발생시간: {alarm.occur_datetime}")
        elif start_datetime and end_datetime and not alarms:
            print(f"⚠️ 날짜 필터 조건에 맞는 데이터가 없습니다.")
            print(f"   필터 조건: {start_datetime} ~ {end_datetime}")
