...
export class FaultDashboardApp {
  ...
  attachTableEvents() {
    ...
    // 🔧 기존 이벤트 리스너 제거 (fault_dashboard.js와의 충돌 방지)
    const tableBody = document.querySelector('.alarm-table tbody');
    if (tableBody) {
      // 기존 이벤트 리스너를 제거하기 위해 복제본으로 교체
    ...
    // 경보 테이블 행 클릭 이벤트 위임 (새로운 tableBody에 적용)
    const newTableContainer = document.querySelector('.table-container');
    newTableContainer.addEventListener('click', (event) => {
  ...
  getCurrentMapData() {
    try {
      ...
      // 1. 먼저 StateManager에서 현재 맵 데이터 조회
      ...
      // 3. 선택된 장비 확인 (맵 데이터가 있는 경우에만 체크)
      const selectedEquipment = StateManager.get('selectedEquipment');
      if (!selectedEquipment) {
        // 🔧 개선: 맵 데이터가 있으면 베이스 노드에서 장비 ID 추출 시도
        const baseNode = mapData.baseNode;
        if (baseNode && (baseNode.equip_id || baseNode.id)) {
          const equipId = baseNode.equip_id || baseNode.id;
          StateManager.set('selectedEquipment', equipId, { source: 'fault-analysis-auto' });
        } else {
  ...
  handleTableRowClick(row) {
    try {
      ...
      // 🔧 수정: 선택된 장비를 StateManager에 설정
      if (equipInfo.equipId) {
        StateManager.set('selectedEquipment', equipInfo.equipId, { source: 'table-row-click' });
        this.showEquipmentSelectedMessage(equipInfo);

        // 🔧 맵이 이미 표시되어 있는지 확인하고, 없다면 맵 로드
