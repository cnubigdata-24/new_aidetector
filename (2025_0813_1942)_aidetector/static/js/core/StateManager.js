...
class StateManager {
  ...
  getCurrentMapData() {
    try {
      ...
      // 이미 enriched된 데이터인지 확인 (alarmMessages와 alarms 필드가 모두 있어야 함)
      const isAlreadyEnriched =
        nodes.length > 0 &&
        nodes[0].hasOwnProperty('alarmMessages') &&
        nodes[0].hasOwnProperty('alarms');
  ...
  enrichMapDataWithAlarms(nodes, links, totalAlarmData) {
    try {
      ...
      const alarmObjectsByEquipId = new Map(); // 전체 경보 객체 저장용
      ...
      mapRelatedAlarms.forEach((alarm) => {
        ...
        if (equipId && alarm.alarm_message) {
          ...
          // 전체 경보 객체 배열
          if (!alarmObjectsByEquipId.has(equipId)) {
            alarmObjectsByEquipId.set(equipId, []);
          }
          alarmObjectsByEquipId.get(equipId).push(alarm);
        }
      });
      ...
      const enrichedNodes = nodes.map((node) => {
        ...
        const alarms = alarmObjectsByEquipId.get(nodeId) || [];

        return {
          ...node,
          alarmMessages: alarmMessages,
          alarms: alarms, // 전체 경보 객체 배열 추가
      ...
      const enrichedLinks = links.map((link) => {
        ...
        const alarms = alarmObjectsByEquipId.get(linkId) || [];
