...
class InferFailurePoint:
    ...
    def _get_mw_alarm_data(self, mw_nodes: List[Dict]) -> List[Dict]:
        ...
        for node in mw_nodes:
            ...
            node_alarms = node.get('alarms', [])
            has_alarm = node.get('hasAlarms', False)
            alarm_count = node.get('alarmCount', 0)

            self.logger.info(
                f"🔍 [디버깅] 노드 {node_name}: hasAlarms={has_alarm}, alarmCount={alarm_count}, 경보수={len(node_alarms)}")
            ...
            for alarm in node_alarms:
                ...
                if is_uas_pattern or is_valid:
                    ...
                    # 필요한 필드들 보완
                    if 'equip_id' not in enhanced_alarm or not enhanced_alarm['equip_id']:
                        enhanced_alarm['equip_id'] = node_id
                    if 'equip_name' not in enhanced_alarm or not enhanced_alarm['equip_name']:
    ...
    def _add_alarm_query_results_text(self, alarm_data: List[Dict]) -> str:
        ...
        if alarm_data:
            # 실제 경보 메시지 표시 (최대 3개까지)
            max_display = min(3, len(alarm_data))
            for i, alarm in enumerate(alarm_data[:max_display]):
                alarm_message = alarm.get('alarm_message', '알 수 없는 경보')

                # 경보 메시지가 너무 길면 축약
                if len(alarm_message) > 50:
                    alarm_message = alarm_message[:47] + "..."

                message += f"{self.HTML_BR}{self.HTML_NBSP}{self.HTML_NBSP} • {alarm_message}"

            # 더 많은 경보가 있는 경우 표시
            if len(alarm_data) > max_display:
                remaining = len(alarm_data) - max_display
                message += f"{self.HTML_BR}{self.HTML_NBSP}{self.HTML_NBSP} • 외 {remaining}건 더..."
